...
export class IssuesService {
  ...
  async create(userId: string, dto: CreateIssueDto) {
    ...
    const issue = await this.prisma.doll_issues.create({
      data: {
        dollId: dto.dollId,
        type: dto.type,
  ...
  async findByDoll(userId: string, dollId: string) {
    ...
    return this.prisma.doll_issues.findMany({
      where: { dollId },
  ...
  async findOne(userId: string, issueId: string) {
    const issue = await this.prisma.doll_issues.findUnique({
      where: { id: issueId },
      include: { dolls: { select: { ownerId: true, fullName: true } } },
    });
    if (!issue) throw new NotFoundException('Signalement introuvable.');
    if (issue.dolls.ownerId !== userId) throw new ForbiddenException('Accès non autorisé.');
    return issue;
  }
  ...
  async update(userId: string, issueId: string, dto: UpdateIssueDto) {
    ...
    return this.prisma.doll_issues.update({
      where: { id: issueId },
      data,
    });
  }
  ...
  async remove(userId: string, issueId: string) {
    ...
    return this.prisma.doll_issues.delete({ where: { id: issueId } });
  }

  async findAllByUser(userId: string) {
    return this.prisma.doll_issues.findMany({
      where: { dolls: { ownerId: userId } },
      include: { dolls: { select: { fullName: true, id: true } } },
